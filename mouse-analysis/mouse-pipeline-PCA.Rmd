...
cleaned_target <- subset(cleaned_target, select = -1)
...
```{r}
seurat_object_norm <- NormalizeData(seurat_object_raw, normalization.method = "LogNormalize", scale.factor = 10000, verbose = FALSE) # Three options are available for normalization - LogNormalize, CenteredLogRatioTransformation, RelativeCounts

seurat_object_norm <- FindVariableFeatures(seurat_object_norm, selection.method = "vst", nfeatures = 2000, verbose = FALSE)
