...
> Run the fastqc on two files

```{bash}
# Run fastqc on all the different samples
...
# mkdir /home/s2599932/human-dataset/GSE36552_ENA_RAW/fastqc-results/multiqc-results
...
> Need to understand the different parameters of trimmomatic

```{bash}
# Use trimmomatic to remove the low quality sequences
...
```{bash}
# Create a reference index (only once)

# mkdir -p /home/s2599932/pre-processing/alignment/fasta
STAR --runThreadN 10 \
...
```{bash}
# Align the reads
mkdir -p /path/to/alignment/output
STAR --runThreadN 12 \
     --genomeDir /home/s2599932/pre-processing/alignment/ref \
     --readFilesIn /home/s2599932/human-dataset/GSE36552_ENA_RAW/trimmed-fastq/SRR490970_trimmed.fastq \
     --outFileNamePrefix /path/to/alignment/output/SRR490970_ \
...
### Use featureCounts to quantify the gene expression
