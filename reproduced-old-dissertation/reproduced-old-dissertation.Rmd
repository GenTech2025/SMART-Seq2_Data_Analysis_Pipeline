...
so_norm_fil_all_genes <- so_norm_fil # this object will be used for PCA using all genes as features
```


```{r pca_using_seurat_object_containing_all_genes_no_explicit_feature_defination, message=FALSE, warning=FALSE}
so_norm_fil_all_genes <- RunPCA(so_norm_fil_all_genes, features = rownames(so_norm_fil_all_genes), verbose = FALSE) # PCA using all genes as features without any explicit feature defination

no_feature_all_genes <- DimPlot(so_norm_fil_all_genes, reduction = "pca", group.by = "Type") +
...
print(no_feature_all_genes)
```


```{r}
no_features_all_genes_load <- Loadings(so_norm_fil_all_genes)
```


### PCA with all genes in the seurat object and mSEGs defined as features.


```{r}
so_norm_fil_mseg <- so_norm_fil # this object will be used for PCA using only mouse stably expressed genes as features
```
...
```{r pca_using_seurat_object_containing_all_genes_no_explicit_feature_defination, message=FALSE, warning=FALSE}
# so_norm_fil_mseg <- subset(so_norm_fil_mseg, features = mSEG)
so_norm_fil_mseg <- RunPCA(so_norm_fil_mseg, verbose = FALSE)
```


```{r fig.height=14, fig.width=14, message=FALSE,warning=FALSE}
DimPlot(so_norm_fil_mseg, reduction = "pca", group.by = "Type") +
      ggtitle("All genes as features") +
      theme_minimal() +
      scale_color_viridis_d()
```
