...
```{r}
data_dir <- "/home/s2599932/study-datasets/human-dataset/E-MTAB-3929/E-MTAB-3929_counts.csv"
targets_dir <- "/home/s2599932/study-datasets/human-dataset/E-MTAB-3929/E-MTAB-3929_targets.csv"
```
...
### Compute the rank difference (OLD)
```{r}
...
### Compute the rank difference (NEW)
#### Find the top 1000 stable genes in the two datasets
```{r}
# Turn the segIdx columns to vectors
sce_one_seg$segIdx <- as.vector(sce_one_seg$segIdx)
sce_two_seg$segIdx <- as.vector(sce_two_seg$segIdx)
...
```{r}
# Remove the genes from the data frame that were unmapped (mostly pseudo genes and non coding RNA) as they cannot be used for fgsea
...
fgsea_one <- fgsea(pathways = msigdb.hs.list, stats = sce_one_ordered1000_lst, minSize =5, maxSize = 100, nPermSimple = 100000)
fgsea_one <- fgsea_one %>%
  arrange(padj, desc(abs(NES)))

# for the second subset
fgsea_two <- fgsea(pathways = msigdb.hs.list, stats = sce_two_ordered1000_lst,minSize =5, maxSize = 100, nPermSimple = 100000)
fgsea_two <- fgsea_two %>%
  arrange(padj, desc(abs(NES)))
```
