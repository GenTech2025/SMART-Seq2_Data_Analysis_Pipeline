...
```{r}
data_dir <- "/home/s2599932/study-datasets/human-dataset/GSE36552/GSE36552_counts.csv"
targets_dir <- "/home/s2599932/study-datasets/human-dataset/GSE36552/GSE36552_targets.csv"
```
...
cell_type_counts <- table(sce_fil$development_stage)
print(length(cell_type_counts))
```
...
ggplot(pca_df_1, aes(x = PC1, y = PC2, color = CellType)) +
  ...
  labs(
    title = "PCA of E-MTAB-3929 using all genes as features",
    x = paste("Principal Component 1 (", all_pc1, "%)", sep = ""),
    y = paste("Principal Component 2 (", all_pc2, "%)", sep = ""),
    color = "Cell Type"
  )
```
...
# Choose the top 250 highest changing genes between the two SCE objects
one_sorted <- sce_one_df_top1000 %>%
  arrange(segIdx_one_vs_two) %>%
  slice_head(n = 250)

two_sorted <- sce_two_df_top1000 %>%
  arrange(segIdx_two_vs_one) %>%
  slice_head(n = 250)

# Store the names of the genes
gene_list_one <- rownames(one_sorted)
gene_list_two <- rownames(two_sorted)

# Load in the human stably expressed gene list from scMerge
data("segList_ensemblGeneID", package = "scMerge")
hSEG <- segList_ensemblGeneID$human$human_scSEG

# Perform the enrichment analysis
genes <- gene_list_one
background_genes <- hSEG


# Verify the overlap between input genes and background genes
common_genes <- intersect(gene_list_one, hSEG)
length_common_genes <- length(common_genes)
cat("Number of common genes:", length_common_genes, "\n")


ego <- enrichGO(
  gene = genes,
  universe = background_genes,      # Custom background list
  OrgDb = org.Hs.eg.db,             # Organism database
  keyType = "ENSEMBL",              # Type of gene ID (ENSEMBL)
  ont = "BP",                       # Ontology (BP: Biological Process, MF: Molecular Function, CC: Cellular Component)
  pAdjustMethod = "BH",             # p-value adjustment method (e.g., "BH" for Benjamini-Hochberg)
...
# Check if `ego` contains results before plotting
if (nrow(ego_df) > 0) {
  # Plot results (bar plot) using ggplot2 for better visualization
  ggplot(ego_df, aes(x = reorder(Description, -Count), y = Count)) +
    geom_bar(stat = "identity") +
    labs(title = "Top GO Terms by Count",
         x = "GO Term",
         y = "Gene Count") +
