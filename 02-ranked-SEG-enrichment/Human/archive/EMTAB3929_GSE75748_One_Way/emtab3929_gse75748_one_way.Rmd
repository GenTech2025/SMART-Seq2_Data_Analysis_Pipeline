...
sce_one_seg_df <- sce_one_seg_df[first_1000_genes,]
sce_two_seg_df <- sce_two_seg_df[first_1000_genes,]
...
> Warning in preparePathwaysAndStats(pathways, stats, minSize, maxSize, gseaParam,  :
  There are ties in the preranked stats (15.82% of the list).
...
# Save as csv files
write.csv(fgsea_one_segidx, file = "./fgsea_one_segidx.csv", row.names = FALSE)
write.csv(fgsea_two_segidx, file = "./fgsea_two_segidx.csv", row.names = FALSE)
write.csv(fgsea_one_rank, file = "./fgsea_one_rank.csv", row.names = FALSE)
write.csv(fgsea_two_rank, file = "./fgsea_two_rank.csv", row.names = FALSE)
```

## Save session data and history
```{r}
save.image(file = "./emtab3929vsgse75748_one_way.RData")
savehistory(file = "./emtab3929vsgse75748_one_way")
writeLines(capture.output(sessionInfo()), "./emtab3929vsgse75748_one_way.txt")
```