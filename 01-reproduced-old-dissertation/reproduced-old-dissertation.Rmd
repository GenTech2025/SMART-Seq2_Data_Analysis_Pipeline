...
so_norm_fil <- ScaleData(so_norm_fil,features = rownames(so_norm_fil), verbose = FALSE) # Adding or removing the features variable does not change anything but this might change after since now the variable features line of code is before it - according to the documentation if findvariablefeatures() is before the scaledata() line and no features are defined in the scaledata() then the scaledata() code will only use the identified variable features.
```
...
```{r save_the_outliers_identified}
write.table(outliers, file = "sample_outliers.csv", sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE)
```
...
p <- VlnPlot(selected_cell_types, features = "ENSMUSG00000031311", group.by = "Type", 
             pt.size = 0.1) +  # Added trim for symmetry
    ...
    theme(
        ...
        panel.grid.minor = element_blank()  # Optional: remove minor grid lines for cleaner look
    ) +
