...
```{r}
outliers_df <- read.csv("../../mouse-analysis-workflows/sample_outliers_SN_NS_NF.csv", header = FALSE)
...
# Create SumOfPCs for both loadings dataframes
subsetted_no_features_defined_loadings <- subsetted_no_features_defined_loadings %>%
  mutate(SumOfPCs = abs(PC_1) + abs(PC_2))
...
top_genes_obj1 <- subsetted_no_features_defined_loadings %>%
  arrange(desc(SumOfPCs)) %>%
  head(25) %>%
  rownames()

top_genes_obj2 <- subsetted_mSEGs_as_features_loadings %>%
  arrange(desc(SumOfPCs)) %>%
  head(25) %>%
  rownames()

# Subset the Seurat objects based on the top 25 genes
subsetted_no_features_defined_subset <- subset(subsetted_no_features_defined, features = top_genes_obj1)
subsetted_mSEGs_as_features_subset <- subset(subsetted_mSEGs_as_features, features = top_genes_obj2)

# Extract the log-normalized counts
lognorm_counts_obj1 <- as.matrix(GetAssayData(subsetted_no_features_defined_subset, slot = "data"))
lognorm_counts_obj2 <- as.matrix(GetAssayData(subsetted_mSEGs_as_features_subset, slot = "data"))

# Center the data by gene (normalize each gene)
lognorm_counts_obj1 <- t(scale(t(lognorm_counts_obj1)))
lognorm_counts_obj2 <- t(scale(t(lognorm_counts_obj2)))

# Transpose the matrices to have genes as columns and samples as rows
lognorm_counts_obj1 <- t(lognorm_counts_obj1)
lognorm_counts_obj2 <- t(lognorm_counts_obj2)
```

```{r fig.height=10, fig.width=12}
# Create heatmaps
# Heatmap for subsetted_no_features_defined_subset
Heatmap(lognorm_counts_obj1, name = "LogNormalized Counts Obj1",
...
Heatmap(lognorm_counts_obj2, name = "LogNormalized Counts Obj2",
        cluster_rows = TRUE, cluster_columns = TRUE,
        show_row_names = FALSE, show_column_names = TRUE,
        heatmap_legend_param = list(title = "LogNormalized Counts"),
        column_title = "Genes", row_title = "Samples")
...
> the objects subsetted_no_features_defined and subsetted_mSEGs_as_features are referred to as obj1 and obj2, respectively
...
```{r fig.height=10, fig.width=12}
## TO MITIGATE THE WARNING AND VISUALIZE THE OUTLYING DATA

# Define custom color scale
col_fun <- colorRamp2(c(min(lognorm_counts_mSEGs), 0, max(lognorm_counts_mSEGs)),
                      c("blue", "white", "red"))

# Generate the heatmap with custom color scale
Heatmap(lognorm_counts_mSEGs, name = "LogNormalized Counts mSEGs",
        cluster_rows = TRUE, cluster_columns = TRUE,
        show_row_names = FALSE, show_column_names = TRUE,
        heatmap_legend_param = list(title = "LogNormalized Counts"),
...
```
```{r}
# Set up the PNG graphics device with the desired dimensions and resolution
png("all_genes_heatmap_mSEGs.png", width = 15, height = 12, units = "in", res = 500)
# Generate the heatmap
# Define custom color scale
Heatmap(lognorm_counts, name = "LogNormalized Counts No Features",
        cluster_rows = TRUE, cluster_columns = TRUE,
